...
from datasets.amazon_reviews import AbstractDataset
from models.rpg.tokenizer import AbstractTokenizer
...
class RPG(nn.Module):
    ...
    def _to_tensor(self, value, dtype=torch.long):
        if torch.is_tensor(value):
            return value.to(self.config['device'])
        if isinstance(value, (list, tuple)) and value and all(torch.is_tensor(item) for item in value):
            stacked = torch.stack([item.to(self.config['device']) for item in value], dim=0)
            if value[0].dim() <= 1:
                stacked = stacked.transpose(0, 1).contiguous()
            return stacked.to(dtype=dtype)
        return torch.as_tensor(value, dtype=dtype, device=self.config['device'])
    ...
    def generate(self, batch, n_return_sequences=1):
        ...
        seq_lens = self._to_tensor(batch['seq_lens'])
